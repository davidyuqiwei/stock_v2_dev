...
def main_download_fuquan_test(getDataFromDB,ProjectDir):
    download_fuquan_schedule(
            getDataFromDB.db_get_hs300_list(if_str=True)[0:5], ProjectDir.download_data_dir_fuquan
        )
    TNLog().info("=== Finish download test fuquan DFCF data =====")
    delete_files_in_folder(ProjectDir.download_data_dir_fuquan)


def main_download_fuquan_all(getDataFromDB,ProjectDir):
    download_fuquan_schedule(getDataFromDB.db_get_hs300_list(if_str=True), ProjectDir.download_data_dir_fuquan)
    """
