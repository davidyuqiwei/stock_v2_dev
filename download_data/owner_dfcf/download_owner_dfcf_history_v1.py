...
class DownloadOwnerDFCF(ProjectDir):
    ...
    def get_url(self, input_date="1", input_page="1"):
        ...
        # 基金
        #url3 = f'https://datacenter-web.eastmoney.com/api/data/v1/get?callback=jQuery112302844694254061386_1734344929915&sortColumns=UPDATE_DATE%2CSECURITY_CODE%2CHOLDER_RANK&sortTypes=-1%2C1%2C1&pageSize=50&pageNumber={input_page}&reportName=RPT_F10_EH_FREEHOLDERS&columns=ALL&source=WEB&client=WEB&filter=(HOLDER_NEWTYPE%3D%22%E5%9F%BA%E9%87%91%22)(END_DATE%3E%3D%27{input_date}%27)'
        #url1 = f'https://datacenter-web.eastmoney.com/api/data/v1/get?callback=jQuery112307065170841873256_1734456423478&sortColumns=UPDATE_DATE%2CSECURITY_CODE%2CHOLDER_RANK&sortTypes=-1%2C1%2C1&pageSize=3000&pageNumber={input_page}&reportName=RPT_F10_EH_FREEHOLDERS&columns=ALL&source=WEB&client=WEB&filter=(END_DATE%3E%3D%27{input_date}%27)'
        #url41 = f'https://datacenter-web.eastmoney.com/api/data/v1/get?callback=jQuery112309886115490425664_1734883289846&sortColumns=UPDATE_DATE%2CSECURITY_CODE%2CHOLDER_RANK&sortTypes=-1%2C1%2C1&pageSize=50&pageNumber=1&reportName=RPT_F10_EH_FREEHOLDERS&columns=ALL&source=WEB&client=WEB&filter=(END_DATE%3D%272022-09-30%27)'
        url1 = f'https://datacenter-web.eastmoney.com/api/data/v1/get?callback=jQuery112309886115490425664_1734883289846&sortColumns=UPDATE_DATE%2CSECURITY_CODE%2CHOLDER_RANK&sortTypes=-1%2C1%2C1&pageSize=3000&pageNumber={input_page}&reportName=RPT_F10_EH_FREEHOLDERS&columns=ALL&source=WEB&client=WEB&filter=(END_DATE%3D%27{input_date}%27)'
        return [url1]

    def download_tmp_txt(self, url_in,url_index,input_date, input_page):
        # input_date = '2024-09-30'
        tmp_data_txt_name_in = os.path.join(
            self.save_tmp_txt_dir, "dfcf_owner_" +str(url_index)+"_"+ input_date + "_" + str(input_page) + ".txt")
        wget.download(url_in,
    ...
    def main_run(self, mode="test"):
        quater_list_raw = get_last_day_of_quarter_range()
        quater_list = [
            x for x in quater_list_raw if '2019' in x or '2018' in x]
        if mode == "test":
            max_page_loop = 5
            quater_list = quater_list[11:13]
        if mode == "all":
            max_page_loop = 200
        print(quater_list)
        url_list = self.get_url()
        for index, url_in in enumerate(url_list):
            for i in quater_list[0:12]:
                for j in range(1, max_page_loop):
                    try:
                        url_in1 = self.get_url(i,str(j))[index]
                        file_size = self.download_tmp_txt(url_in1,index,i, j)
                        if file_size < 1:
                            print("\n===========\n")
                            print(i)
                            break
                        ...
                        TNLog().error("=========== download_owner_dfcf_error ============")
                        TNLog().error(e)
    ...
    def t_case(self):
        a1 = self.get_url("2024-09-30", "")
        print(a1)

if __name__ == "__main__":
    #DownloadOwnerDFCF(ProjectDir).t_case()
    a1 = DownloadOwnerDFCF(ProjectDir).main_run("all")
    #DownloadOwnerDFCF(ProjectDir).combine_owner_dfcf_data()

    # a1 = DownloadOwnerDFCF(ProjectDir)
    # input_page = '80'
    # input_date = '2022-09-30'
    # url1 = f'https://datacenter-web.eastmoney.com/api/data/v1/get?callback=jQuery112309886115490425664_1734883289846&sortColumns=UPDATE_DATE%2CSECURITY_CODE%2CHOLDER_RANK&sortTypes=-1%2C1%2C1&pageSize=3000&pageNumber={input_page}&reportName=RPT_F10_EH_FREEHOLDERS&columns=ALL&source=WEB&client=WEB&filter=(END_DATE%3D%27{input_date}%27)'
    # file_size = a1.download_tmp_txt(url1,0, input_date, input_page)
    # print("======\n")
    # print(file_size)
    ...
    # a1 = DownloadOwnerDFCF(ProjectDir)
    # delete_files_in_folder(a1.save_tmp_txt_dir)
